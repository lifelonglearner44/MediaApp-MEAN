...
export class MockXHRBackend implements HttpBackend {
  ...
  _deleteMediaItem(id) {
    const mediaItem = this.mediaItems.find(i => i.id === id) as { id: number; name: string; medium: string; category: string; year: number; watchedOn: number | null; isFavorite: boolean; };
